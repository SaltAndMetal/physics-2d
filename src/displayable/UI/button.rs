...
impl Displayable for Button
{
    ...
    fn display(&self, canvas: &mut Canvas<Window>)    {
        ...
        for (x, y) in self.texture.coordinates() {
            ...
            canvas.set_draw_color(Color::RGB(px.r, px.g, px.b));
            canvas.draw_point(Point::from(self.topLeft)+Point::new((x as f64*scale.x()) as i32, (y as f64*scale.y() as f64) as i32)).unwrap();
